...
- `/authority` binds the operator-supplied starting commit/tree, bootstrap
  ...
  states, EV-0016 evidence tail, D33 study boundary, every protected hash, and
  the additive Gate A operator amendment without modifying the original prompt.
...
- `/prompt_arms` defines NT, PT, WT, and ST and explicitly excludes generated
  chain of thought.
- `/stage_machine` fixes P → T → B-D → B-C → M-D → M-C separation. B-D contains
  the amended fixed Gate A decision before B-C can be opened. Every stage after
  P requires new authority.
...
- `/design_sensitivity` binds the model-free ACR power calculation without
  changing any registered sample size or threshold.
- `/feasibility_gate` fixes the target-only, two-family development Gate A that
  may open B-C or close protocol v1 but cannot revise it.
- `/execution_reliability` fixes non-scientific sharding, retry, resume,
  duplicate-rejection, and manifest-last requirements for later authorities.
- `/advisory_boundaries` keeps post-hoc Study 1 analysis and the four-scale
  J-lens observation outside scientific evidence and selection.
...
## Design sensitivity and Gate A

The exact model-free sensitivity pack is
`stage_p_power_sensitivity.json`, generated by ACR QuickRun `cmc2`. It records
exact binomial power for n=64, n=128, and n=256, the probability of clearing
the confirmation accuracy floor under several true accuracies, paired
target-control sensitivity under fixed discordant-pair proportions, and
standardized paired-effect sensitivity for n=128 mechanistic pairs. These are
marginal design calculations; joint power across the conjunctive scientific
gates is not identified.

After Stage T and B-D implementation validation, but before any confirmation
object is opened, all three models run the complete fixed development pack.
Gate A uses only target NT rows and pools the fixed depth-2 and depth-3 cells
within each family, giving n=128 per family. A family passes when execution and
balance are complete and its exact one-sided binomial upper tail against .25 is
at most .025. This is exactly 43 or more correct rows; 42 does not pass. Both
families must pass, with no pooled, depth, or control-model rescue.

Passing seals the development decision but still requires separate authority
to open B-C. Failure closes protocol v1 as
`STUDY2_PROTOCOL_V1_CLOSED_ON_DEVELOPMENT_FEASIBILITY`; it is not a scientific
negative. A later attempt would require a new protocol version, authority, and
seeds. Same-version backfill, task replacement, or threshold change is
forbidden.

## Later execution reliability

Later authorities must bind immutable shard manifests and idempotent attempt
IDs, perform capacity preflight, allow only finite reason-code-bound retries,
write create-only checkpoints, verify resume without duplicate contribution,
isolate failed shards without shrinking denominators, reject duplicate,
missing, and out-of-shard rows, and merge a complete pack manifest-last.
Infrastructure exhaustion is operationally not estimable, never a scientific
negative. GPU cost cannot reduce samples, repetitions, or critical controls.

Any Claude Study 1 failure analysis remains
`ADVISORY_POST_HOC_METHODS_INPUT`. The protected four-scale J-lens engineering
trajectory remains
`EXPLORATORY_METHODS_OBSERVATION_AWAITING_CONTROLLED_VALIDATION`; it makes no
publication, novelty, or `max_seq_len=128` causal claim and has no primary
selection role.
...
`/output_contract`. The future feasibility row binds every Gate A count,
integrity input, decision, and confirmation-unopened assertion. Every future
behavior row binds the model/prompt/token
