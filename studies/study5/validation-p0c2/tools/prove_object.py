...
def main() -> int:
    ...
    accuracy_floor = float(tightening["requirement_4_accuracy_floor"]["now"])
    ablation_ceiling = float(tightening["requirement_3_ablation_ceiling"]["now"])
    required_drop = float(tightening["requirement_3_drop_floor"]["now"].split()[-1])
    chance = 1.0 / 9.0
        ...
        per_item.append(
            {
                ...
                "role": item["role"],
                "registration_line_position": item["registration_line_position"],
    ...
    report = {
        "schema_version": "study5-p0c2-object-proof-v1",
        "phase": "P-0c-2",
        ...
        "requirement_3_anti_retrieval": {
            "probe": "the same prompt with the queried name's own registration line removed",
            "why_it_is_the_right_probe": "it removes the FIRST hop's input while leaving the second hop's clue, the rules table, entirely intact",
            ...
            "comparison": "drop >= required_drop; the floor is the value P-0c measured, so this object must be at least as anti-retrievable as its predecessor",
        ...
        "verbatim": (
            reg["5_conclusion_wordings_fixed_before_any_forward_pass"][
    ...
    if not established:
        print("P0C2-CHECK-OBJECT-PROOF FAILED", file=sys.stderr)
        return 1
    print("P0C2-CHECK-OBJECT-PROOF PASSED")
    return 0
