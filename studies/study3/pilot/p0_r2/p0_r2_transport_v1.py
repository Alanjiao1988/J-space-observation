#!/usr/bin/env python3
"""Recover P0-R2 envelopes after checksum-verifiable ACR log interleaving."""
...
import p0_r2_transport as BASE  # noqa: E402

SCHEMA_VERSION = "study3-p0-r2-acr-log-recovery-v1"
...
def recover_with_report(log_text, attempt_id=None):
    """Use the base decoder first; repair only a checksum-proved split."""
    try:
        return BASE.recover(log_text, attempt_id=attempt_id), []
    except BASE.TransportDefect as original:
        repaired, repairs = repair_acr_fragments(log_text)
        if not repairs:
            raise original
        recovered = BASE.recover(repaired, attempt_id=attempt_id)
        return recovered, repairs


def recover(log_text, attempt_id=None):
    return recover_with_report(log_text, attempt_id=attempt_id)[0]
...
def main(argv=None):
    ...
    try:
        ...
        print("P0_R2_TRANSPORT_V1_REFUSED=1 %s" % exc, file=sys.stderr)
        return 3
    for entry in written:
        print("RECOVERED=%s BYTES=%d SHA256=%s"
              % (entry["name"], entry["bytes"], entry["sha256"]))
    print("P0_R2_TRANSPORT_V1_RECOVERY_COMPLETE=1 REPAIRS=%d" % len(repairs))
    return 0
