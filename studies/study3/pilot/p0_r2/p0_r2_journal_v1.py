...
P0-R1 generation 2 had a journal, and on the production command it wrote nowhere
...
P0-R2 generation 1 therefore fixes three things at once:
...
SCHEMA_VERSION = "study3-p0-r2-durable-journal-v1"
ATTEMPT_ID_PREFIX = "p0r2-g1-"

JOURNAL_DIRECTORY = "journal"
MANIFEST_NAME = "p0_r2_journal_manifest_v1.json"
...
def validate_attempt_id(attempt_id):
    if not isinstance(attempt_id, str) or not attempt_id.startswith(
            ATTEMPT_ID_PREFIX):
        raise JournalDefect(
            "journal attempt id %r does not begin with registered %r"
            % (attempt_id, ATTEMPT_ID_PREFIX))
    if ".." in attempt_id or any(
            not (character.isalnum() or character in "-_.")
            for character in attempt_id):
        raise JournalDefect(
            "journal attempt id %r is not a safe component" % attempt_id)
    return attempt_id
...
def _merge_extra(document, extra):
    ...
    if overlap:
        raise JournalDefect(
            "extra journal metadata may not replace bound field(s): %s"
            % ", ".join(overlap))
...
class DurableJournal(object):
    ...
    def _emit(self, kind, body):
        ...
        overlap = sorted(set(entry).intersection(body))
        if overlap:
            raise JournalDefect(
                "journal body may not replace bound field(s): %s"
                % ", ".join(overlap))
        entry.update(body)
        ...
        if self.stream is not None:
            self.stream.write("P0R2JRN seq=%d kind=%s name=%s sha256=%s\n"
    ...
    def complete(self, admission, payload=None):
        ...
        if payload is None:
            raise JournalDefect(
                "a completion must retain the complete observed payload")
    ...
    def fail(self, admission, error, payload=None):
        ...
        if opened is None:
            raise JournalDefect(
                "admission %r is not open; a failure must follow exactly one "
                "admission" % (admission,))
        return self._emit("completion", {
            "operation": opened["operation"],
            "state": FAILED,
    ...
    def manifest(self, canonical=None, extra=None):
        ...
        P0-R1 generation 2 enumerated the output directory with
        ``os.listdir``, which returns only top-level names and therefore
        silently excluded the whole nested journal. A manifest that cannot see
        the evidence cannot verify it.
        """
...
def verify_manifest(manifest, sink):
    ...
    validate_attempt_id(manifest.get("attempt_id"))
    if manifest.get("written_last") is not True \
            or manifest.get("recursive_enumeration") is not True:
        raise JournalDefect(
            "the journal manifest does not prove recursive, final enumeration")
    listed = manifest.get("journal_objects")
    if not isinstance(listed, list):
        raise JournalDefect("the journal manifest object list is not a list")
    if manifest.get("journal_object_count") != len(listed):
        raise JournalDefect(
            "the journal manifest object count does not reconcile")
    ...
    for entry in listed:
        if not isinstance(entry, dict):
            raise JournalDefect(
                "the journal manifest contains a non-object index entry")
        name = entry.get("name")
        ...
        if name != sequence_name(entry["sequence"], entry.get("kind")):
            raise JournalDefect(
                "journal object %r does not match its sequence and kind" % name)
        expected_sequence += 1
        ...
        try:
            ...
            raise JournalDefect(
                "journal object %r is not UTF-8 JSON: %s" % (name, exc))
        if not isinstance(document, dict) \
                or document.get("schema_version") != SCHEMA_VERSION \
                or document.get("attempt_id") != manifest.get("attempt_id") \
                or document.get("sequence") != entry.get("sequence") \
                or document.get("kind") != entry.get("kind"):
            raise JournalDefect(
                "journal object %r does not match its bound index entry" % name)
...
def implementation_identity(root=None):
    return {
        ...
        "closes_predecessor_defects": "P0-R1 G2-07, G2-08",
    }
...
def main(argv=None):
    ...
    if args.self_check:
        ...
        import p0_r2_blob_transport_v1 as BLOB_V1
        transport = BLOB_V1.PrivateBlobTransportV1(
            "p0r2-g1-journal-self-check", backend=BLOB.InMemoryBackend())
        journal = DurableJournal("p0r2-g1-journal-self-check",
        ...
        print("P0_R2_JOURNAL_V1_SELF_CHECK=passed OBJECTS=%d SEQUENCES=%d"
