"""Study 3 P0-R2 generation-1 complete-byte transport envelope.

Authority:
``studies/study3/prompts/study3_p0_r2_infrastructure_successor_authority.md``
section 6, over the two earlier P0-R2 authorities.

The terminal P0-R1 predecessor printed each replay artifact's SHA-256 and byte
count and nothing else. A digest and a byte count cannot reconstruct the
preimage, so the replay result was recoverable only by rerunning the one-shot
gate, which is exactly what the earlier authority forbade. This P0-R2 module
closes that defect without changing or reopening P0-R1.
...
ENVELOPE_VERSION = "study3-p0-r2-transport-envelope-v1"

CHUNK_MARKER = "P0R2TXC"
MANIFEST_MARKER = "P0R2TXM"
COMPLETE_MARKER = "P0R2TXE"

#: Every P0-R2 generation-1 attempt is syntactically disjoint from P0-R1.
...
def validate_attempt_id(attempt_id):
    """Require the preregistered P0-R2 generation-1 attempt namespace."""
    ...
    if len(attempt_id.encode("utf-8")) > MAX_ATTEMPT_ID_BYTES:
        raise TransportDefect(
            "attempt id %r exceeds the registered %d-byte ceiling"
            % (attempt_id, MAX_ATTEMPT_ID_BYTES))
    if ".." in attempt_id or any(
            not (character.isalnum() or character in "-_.")
            for character in attempt_id):
        raise TransportDefect(
            "attempt id %r is not a safe envelope component" % attempt_id)
    return attempt_id
...
def encode_artifact(attempt_id, name, payload):
    ...
    if len(payload) > MAX_PROJECTED_COMBINED_REPLAY_ARTIFACT_BYTES:
        raise TransportDefect(
            "%s is %d bytes, above the registered %d-byte per-artifact "
            "transport ceiling"
            % (name, len(payload),
...
def manifest_document(attempt_id, artifacts):
    ...
    validate_attempt_id(attempt_id)
    if set(artifacts) != set(REPLAY_ARTIFACTS):
        raise TransportDefect(
            "an envelope must carry the exact canonical artifact set; "
            "missing=%s extra=%s"
            % (sorted(set(REPLAY_ARTIFACTS) - set(artifacts)),
               sorted(set(artifacts) - set(REPLAY_ARTIFACTS))))
    entries = []
    for name in sorted(artifacts):
        ...
        if len(payload) > MAX_PROJECTED_COMBINED_REPLAY_ARTIFACT_BYTES:
            raise TransportDefect(
                "%s is above the registered per-artifact transport ceiling"
                % name)
        entries.append({
            "bytes": len(payload),
...
def encode(attempt_id, artifacts):
    ...
    # Build the document first so the exact four-name contract and every
    # payload bound are checked before a single envelope line is returned.
...
def recover(log_text, attempt_id=None):
    ...
    validate_attempt_id(observed_attempt)
    ...
    for position, marker, fields in records:
        ...
        if marker == CHUNK_MARKER:
            name = validate_artifact_name(fields["n"])
            ...
            if index >= total:
                raise TransportDefect(
                    ...
                    % (index, name, total))
            if byte_count > MAX_PROJECTED_COMBINED_REPLAY_ARTIFACT_BYTES:
                raise TransportDefect(
                    "%s declares %d bytes, above the registered %d-byte "
                    "per-artifact transport ceiling"
                    % (name, byte_count,
                       MAX_PROJECTED_COMBINED_REPLAY_ARTIFACT_BYTES))
            expected_total = chunk_count(byte_count)
            if total != expected_total:
                raise TransportDefect(
                    "%s declares %d chunks for %d bytes; exactly %d is "
                    "required" % (name, total, byte_count, expected_total))
            try:
            ...
            expected_raw_size = min(
                RAW_CHUNK_BYTES,
                max(0, byte_count - index * RAW_CHUNK_BYTES))
            if len(raw) != expected_raw_size:
                raise TransportDefect(
                    "chunk %d of %s carries %d raw bytes, not the required %d"
                    % (index, name, len(raw), expected_raw_size))
    ...
    if set(manifest) != set(REPLAY_ARTIFACTS):
        raise TransportDefect(
            "the captured envelope does not carry the exact canonical "
            "artifact set; missing=%s extra=%s"
            % (sorted(set(REPLAY_ARTIFACTS) - set(manifest)),
               sorted(set(manifest) - set(REPLAY_ARTIFACTS))))
...
def write_recovered(recovered, out_dir):
    """Write the exact canonical files into an operator-owned directory."""
    if set(recovered) != set(REPLAY_ARTIFACTS):
        raise TransportDefect(
            "recovery output must contain the exact canonical artifact set")
    if os.path.islink(out_dir):
        raise TransportDefect("the recovery output directory may not be a symlink")
    ...
    for name in sorted(recovered):
        ...
        try:
            ...
            raise TransportDefect(
                "refusing to overwrite existing recovered artifact %s" % path)
        written.append({
            "name": name,
...
def reconstruction_receipt(attempt_id, recovered, log_identity=None,
    ...
    return {
        "schema_version": "study3-p0-r2-transport-reconstruction-receipt-v1",
        "document_class": "study3_p0_r2_transport_reconstruction_receipt",
...
def self_check(attempt_id="p0r2-g1-transport-self-check"):
...
def implementation_identity(root=None):
    """The transport implementation's own byte identity for the P0-R2 lock."""
    path = os.path.abspath(__file__) if root is None else os.path.join(
        root, "studies", "study3", "pilot", "p0_r2", "p0_r2_transport.py")
    with open(path, "rb") as handle:
        raw = handle.read()
    return {
        "path": "studies/study3/pilot/p0_r2/p0_r2_transport.py",
        "bytes": len(raw),
        "sha256": _sha256(raw),
        "envelope_version": ENVELOPE_VERSION,
...
def main(argv=None):
    ...
    try:
        if args.self_check:
            report = self_check()
            print("P0_R2_TRANSPORT_SELF_CHECK=1")
        ...
        if args.recover:
            if not args.log or not args.out_dir or not args.attempt:
                raise TransportDefect(
                    "--recover requires --log, --out-dir and --attempt")
            ...
            for entry in written:
                print("RECOVERED=%s BYTES=%d SHA256=%s"
                      % (entry["name"], entry["bytes"], entry["sha256"]))
            print("P0_R2_TRANSPORT_RECOVERY_COMPLETE=1")
            return 0

        parser.print_help()
        return 2
    except (OSError, TransportDefect) as exc:
