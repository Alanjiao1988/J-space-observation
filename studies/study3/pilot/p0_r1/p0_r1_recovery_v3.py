...
def recover(attempt_id, backend=None, lock_sha256=None, stream=None):
    ...
    try:
        ...
        raise RecoveryDefect(
            "the journal manifest %s could not be read under %s: %s"
            % (JOURNAL.MANIFEST_NAME, transport.prefix, exc))
    try:
        manifest = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, ValueError) as exc:
        raise RecoveryDefect("the journal manifest is not readable JSON: %s"
                             % exc)
    ...
    for entry in verified["objects"]:
        payloads[entry["name"].replace("/", "__")] = sink.read(entry["name"])
    ...
    receipt = {
        ...
        "manifest_written_last": verified["manifest_written_last"],
