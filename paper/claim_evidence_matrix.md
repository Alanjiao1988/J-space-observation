...
| Available evidence | `EV-0001`, `EV-0005`, and `EV-0009` establish bounded engineering feasibility and show that early 10/25-prompt lenses were not stable. `EV-0015` adds two disjoint full-layer 600-sequence fits and the exact 50:50 M1200 merge: all artifacts are finite, losslessly serialized, independently byte-sealed, and complete on the registered heldout diagnostics; the A/B maximum relative Frobenius distance decreases to 0.073860 at n=600. `EV-0016` executes the lens-free S3 E0 eligibility gate and stops with no confirmation sample. None of these rows contains a semantic readout or causal lens result. |
| Missing evidence | Every functional-validity quantity: M1200/A600/B600 and logit-lens readout effects, leakage controls, coordinate swaps, ablation, matched random directions, lens-independent patching support, confirmation bootstrap intervals, and a valid classification. Study 1 is closed, so those missing quantities will not be produced under its authority. |
| Status | **unsupported** |
| Key artifacts | `artifacts/jlens-s2-production/20260806T194226Z/`; `docs/jlens_s3_validity_protocol.json`; `artifacts/jlens-s3-e0/20260807T081017Z/`; `docs/jlens_s2_s3_e0_final_handoff.md`. |
| Limitations | Two 600-sequence fits provide one A/B difference trajectory, not a sampling distribution. WikiText is only a public pretraining-like proxy. Matrix distance, heldout logit stability, exact merging, and byte seals are engineering properties, not semantic or causal validity. E0 opened no lens, and its interface-specific floor failure cannot be converted into a J-lens result (`L-62`–`L-67`). |
...
| Available evidence | `EV-0015` supplies exact sealed A600, B600, and M1200 engineering identities. `EV-0016` supplies the complete frozen E0 eligibility result: 2/93 multihop, 2/55 order-operations, and 5/90 causal-swap items were behaviorally eligible; the development-first split left every confirmation count at zero. E0 loaded or applied no lens. Therefore there is still no empirical validity evidence. |
| Missing evidence | All E1/E2 functional evidence: readout curves, log-AUC comparisons, label/position/answer leakage controls, causal swaps, ablation, random-direction controls, activation patching, bootstrap intervals, and a confirmation classification. These quantities are now permanently not estimable under Study 1. |
