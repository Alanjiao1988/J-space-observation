"""Deterministic, reference-blind numeric extraction for parser-v2.

``parse_v2`` accepts only the frozen three-field request.  Correctness is kept
...
PARSER_SOURCE_SHA256 = "f538add0bdd6e5a3281d0298b374a99fecea962a91a4cbaa5b4a20795d9a6918"
PARSER_VERSION = "6cfaec62db37562930a4cb7d3a252bcbf80e1eaf748de98213863ff2566a7f86"

PARSER_SOURCE_DIGEST_DOMAIN = b"jspace-parser-v2/source/v1\0"
PARSER_VERSION_DIGEST_DOMAIN = b"jspace-parser-v2/version/v1\0"
PARSER_ALGORITHM_ID = "jspace-parser-v2-reference-blind-extraction/v1"
...
_HARMLESS_RESIDUAL_PATTERN = re.compile(r"[\s.,;:!?()\[\]{}$\\'\"`]*\Z")
...
_UNIT_WORD_PATTERN = re.compile(
    r"\s*(?:kg|g|mg|km|cm|mm|m|s|ms|hz|kb|mb|gb|bytes?)\b",
    re.IGNORECASE | re.ASCII,
)
...
def _claim_has_invalid_continuation(
    text: str, token_end: int
) -> bool:
    cursor, _ = _skip_whitespace(text, token_end)
    if cursor >= len(text):
        return False
    if text[cursor] in "+*/":
        return True
    if (
        ...
        return True
    if _UNIT_WORD_PATTERN.match(text, token_end):
        return True
    return False
...
def _scan_markers(
    ...
    for match in _MARKER_LABEL_PATTERN.finditer(visible_text):
        ...
        cursor, had_whitespace = _skip_whitespace(visible_text, match.end())
        ...
        if cursor < len(visible_text) and visible_text[cursor] in ":：=":
            ...
            family == "answer"
            and label_text == "answer"
            and visible_text[cursor : cursor + 2].casefold() == "is"
            and (
                cursor + 2 == len(visible_text)
                or not visible_text[cursor + 2].isalnum()
            )
        ):
            cursor += 2
            cursor, _ = _skip_whitespace(visible_text, cursor)
            if cursor < len(visible_text) and visible_text[cursor] in ":：=":
                count = 0
                while (
                    cursor < len(visible_text)
                    and visible_text[cursor] in ":：="
                ):
                    count += 1
                    cursor += 1
                ...
                cursor, _ = _skip_whitespace(visible_text, cursor)
            has_separator = True
        elif had_whitespace and (
            cursor == len(visible_text)
            or _NUMERIC_AT_PATTERN.match(visible_text, cursor)
            or visible_text.startswith(r"\boxed", cursor)
        ):
            has_separator = True
        ...
        if not incomplete and not wraps_box:
            ...
            if numeric_match is not None:
                ...
                if re.match(
                    ...
                    invalid_complete = True
                elif not payload or payload.startswith(("?", "...", "…")):
                    incomplete = True
...
def _terminal_suffix_is_harmless(
    ...
    suffix = line[end:]
    return bool(re.fullmatch(r"[\s.,;:!?()[\]{}$\\]*", suffix))
...
def _scan_terminal_equations(
    ...
    for match in re.finditer(r"(?<![<>=])=(?!=)", line, re.ASCII):
        if not _equation_lhs_is_supported(line, match.start()):
            continue
        cursor, _ = _skip_whitespace(line, match.end())
        if cursor >= len(line):
            incomplete = True
            continue
...
def parse_v2(request: Mapping[str, Any]) -> dict[str, Any]:
