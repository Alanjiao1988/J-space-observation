...
reproduced the already-committed public anchor `e1364afc…`. The receipt records
twelve invariants checked and none failed; that count is a literal the probe
wrote, and what the assessor independently recomputes is the aggregate digest,
the object count and the byte total.
...
emits, and what makes them credible is an AST check over the two first-party
Python files that run inside the job — the probe and the receipt validator —
that fails the build if a decode, persist or write call appears in either. The
Azure SDK, the standard library and the base image are not parsed.
...
network rules encode. No qualifying backend was found within the enumerated
search scope: the executable boundary assessment
...
> **Superseded by the Phase 1.2G handoff at the end of this file.** Phase 1.2G
> took the instrument-strictness decision this section named as its next
> action, so this section's "Next authorized action" and its "What to know
> before the next round" bullet on the calibration protocol no longer describe
> the current state. Everything else here stands as history.
...
# HANDOFF - Phase 1.2G complete, READY_FOR_INDEPENDENT_SET_REPAIR (supersedes the Phase 1.2F handoff above)

## What this round settled

Phase 1.2F ended blocked on one scientific decision and left ten consistency
defects behind it. Phase 1.2G took the decision and remediated the defects.

The decision is **strict finite-suite conformance**. The future `parser-v3-v2`
set is a finite conformance suite, not a sample from a deployment population.
A case is admitted to it precisely because a correct instrument is required to
handle it. Every admitted case is therefore a mandatory conformance example,
and an exact typed-decision mismatch on any one of them is unacceptable
instrument behaviour. Zero residual tolerance follows as a `LOGICAL_INVARIANT`.

Read that derivation carefully before touching it. It does **not** rest on the
parser being deterministic, on the set not being IID, on calibration being
unavailable, on conservatism, on tidiness, on parser-v2 precedent, or on any
expectation about how parser v3 will perform. A regression test scans the
derivation text for exactly those appeals and fails if one appears. The premise
is recorded together with its falsifier: if a case is ever admitted that a
correct instrument is not required to handle, the premise fails and the
invariant loses its basis.

Do not reopen the zero-versus-non-zero question as though it were still open.
It was settled by operator decision in this round.
...
Prospective acceptance policy     FINAL
Sealed inputs read                0
Sealed labels read                0
...
`READY_FOR_INDEPENDENT_SET_REPAIR` authorizes **one** thing: a separately
authorized set-repair round. It is not a result, not an evaluation, and not a
statement about parser v3.

## Threshold outcome

| Threshold | Disposition | Value | Basis |
| --- | --- | --- | --- |
| `overall_exact_typed_decision_minimum` | `REPLACE_HARD` | - | - |
| `critical_stratum_floor` | `MERGE_WITH_EXISTING_GATE` | - | - |
| `answer_presence_macro_f1_minimum` | `REPORT_ONLY` | - | - |
| `non_regression_margin_vs_parser_v2` | `REPORT_ONLY` | - | - |
| `residual_critical_exact_budget` | `KEEP_HARD` | `0` pooled, `0` per stratum | `LOGICAL_INVARIANT` |

With 80 of 120 cases pinned by mandatory gates and the residual 40 at zero,
every case in the set is now pinned to exact typed-decision agreement. Neither
of the first two metrics is *deleted*: the protection each one aimed at survives
inside `residual_critical_exact_budget` and the mandatory gates respectively,
which is why they are recorded as replaced and merged rather than removed. The
canonical labels are the ones in
`docs/phase1_parser_v3_v2_evaluation_policy.json`. Macro F1 stays report-only
because an exhaustive enumeration of the 861 feasible three-class confusion
matrices at the registered supports shows it cannot detect a wrong canonical
value that preserves the presence class.

## What to know before the next round

* **The policy JSON is the canonical source of truth.**
  `docs/phase1_parser_v3_v2_evaluation_policy.json` governs. Where a report,
  protocol, ledger or code comment disagrees with it, the other artifact is
  defective. It declares this about itself, and the validator enforces the
  parts that are machine-checkable.
* **There is one production coverage derivation.**
  `jspace_observation.parser_v3_repair_contract.derive_gate_coverage` computes
  exact-typed-decision coverage from each gate's registered error definition.
  Validation and compilation both consume it. The policy's own
  `gate_coverage_analysis` block is a *restatement* that the validator requires
  to agree with the derivation - it is not an input. Do not add a second
  implementation.
* **`GATE_ERROR_DEFINITIONS` is code-owned and closed.** The registry in the
  production module is the authority on what each gate's error definition means
  and whether it pins exact typed-decision agreement. The policy's Boolean is a
  restatement that must agree. An unknown definition is refused.
* **Current-state figures are generated, not typed.**
  `scripts/generate_current_state.py --check` re-renders the current-state
  block from the canonical policy and compares bytes. If you change the policy,
  run it with `--write`. Six of this round's ten seed defects were stale
  figures that no prose pattern covered; this is the structural answer to that
  class of defect, and the prose scanner
  `scripts/check_current_state_consistency.py` remains as the backstop for
  claims that are not figures.
* **Both a pooled limit and per-stratum limits are stated.** A pooled zero
  alone would have been satisfiable by a later policy that raised one stratum's
  cap while holding the pool. Both are recorded so neither can drift.
* **A zero margin is still a margin.** The comparator validator distinguishes
  `is not None` from truthiness, because `0 == False` in Python and a margin of
  zero encodes the substantive rule "must not be worse than parser v2". This
  was a real defect in this round's own new code, found by its own new test.
* **`G-01` to `G-10` are consistency defects, not scientific findings.** They
  are deliberately not numbered into the historical `H1`-`H9` series, which is
  unchanged. `H9` remains specifically about disagreement among declared and
  observed artifact vocabulary, support and set facts.
* **The calibration protocol is `SUPERSEDED_UNEXECUTED`.** It was registered in
  Phase 1.2F as the fallback for a non-zero tolerance. That branch never became
  live. It was never executed, and it is not the next gate. Its own header
  records the four conditions under which it would reactivate.
* **The parser-isolation claim is bounded, unchanged from Phase 1.2F.**
  `jspace_observation/__init__.py` eagerly imports the legacy parser, so package
  import is not parser-free and no document may say it is. The supportable
  claims are that the repair modules introduce no new parser dependency,
  contain no parser symbol reference, and invoke no parser. Do not refactor
  `__init__.py` to make a stronger claim true.
* **`L-32` still holds.** A sealed member list and `sealed_object_count` require
  an authenticated seal-time observation. No offline document derives them, and
  operator assertions must not masquerade as set-derived facts.
* **Independent audits are recorded in
  `reports/phase1_2g_audit_findings.md`,** including a post-remediation
  re-review, because an audit that only sees the pre-fix state plus the
  author's own claim of remediation is not independent verification.

## Standing statements that every current-state document must carry

These are rendered mechanically into the generated current-state block at the
top of this file. Restating them by hand elsewhere is allowed; contradicting
them is not.

* Phase 1.0C was **executed** and finalized **`INCONCLUSIVE`**.
* Phase 1.0C is **not** parser calibration. It is target-model
  observable-answer task/headroom screening, and no Phase 1.0C result can
  supply, bound, or unblock any parser acceptance threshold.
* **No private holdout was accessed in Phase 1.2G.** No sealed input, sealed
  label, private curator file, answer value, output text, span, offset, case
  identity or case-level label was read.
* **No prediction was generated.**
* **No parser was run** on any evaluation or calibration corpus.
* **No formal evaluation occurred.** The formal parser-v3 evaluation ordinal
  remains `0`, parser-v3 predictions against a locked set remain `0`, and
  locked-label reads remain `0`.
* **Parser v3 remains unvalidated.** It is not improved, not non-regressive,
  not accepted, and not fit for scientific scoring. The policy became stricter
  in this round, not more permissive.
* **`parser-v3-v1` remains retired and unchanged** -
...
Nothing else is authorized. Not case construction, not migration of the 105
cases, not review or replacement of the 15 quarantined cases, not manifest
generation, not sealing, not authorization locks, not image construction, not
preregistration, not Stage P, not Stage E.
