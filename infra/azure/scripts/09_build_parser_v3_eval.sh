...
if [[ "${JSPACE_PV2_BUILD_VERIFIED_REEXEC:-}" == "1" ]]; then
    ...
    expected_build_script="$SNAPSHOT_SOURCE_ROOT/infra/azure/scripts/09_build_parser_v3_eval.sh"
    ...
    if [[ "$current_build_script" != "$expected_build_script" \
        ...
        || ! "$(basename "$BUILD_SNAPSHOT_DIR")" \
            =~ ^parser-v3-build-inputs-[0-9a-f]{32}$ ]]; then
...
AZURE_HELPER="$HELPER_ROOT/scripts/parser_v3_azure_contract.py"
...
if [[ "${JSPACE_PV2_BUILD_VERIFIED_REEXEC:-}" != "1" ]]; then
    ...
    git_private_root="$(git -C "$PROJECT_ROOT" rev-parse --absolute-git-dir)"
    BUILD_SNAPSHOT_DIR="$git_private_root/parser-v3-build-inputs-${snapshot_nonce}"
    SNAPSHOT_SOURCE_ROOT="$BUILD_SNAPSHOT_DIR/sources"
...
PY
    snapshot_build_script="$SNAPSHOT_SOURCE_ROOT/infra/azure/scripts/09_build_parser_v3_eval.sh"
...
STAGING_TAG=""
RECORD_DIR="${PARSER_EVAL_BUILD_RECORD_DIR:-$PROJECT_ROOT/results/runs/parser-v3-eval-build-${SOURCE_SHA}}"
...
for script in \
    ...
    "scripts/stage_p_v3_entrypoint.sh" \
    "scripts/stage_p_adopt_v3_entrypoint.sh" \
    "scripts/stage_e_v3_entrypoint.sh"; do
