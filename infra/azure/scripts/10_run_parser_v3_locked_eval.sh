...
if [[ "${JSPACE_PV2_VERIFIED_REEXEC:-}" == "1" ]]; then
    ...
    AZURE_HELPER="$SNAPSHOT_SOURCE_ROOT/scripts/parser_v3_azure_contract.py"
    ...
    AZURE_HELPER="$PROJECT_ROOT/scripts/parser_v3_azure_contract.py"
...
if [[ "${JSPACE_PV2_VERIFIED_REEXEC:-}" == "1" ]]; then
    ...
    EXPECTED_LAUNCHER_PATH="$SNAPSHOT_PARENT/$SNAPSHOT_BASENAME/sources/infra/azure/scripts/10_run_parser_v3_locked_eval.sh"
...
PY
    SNAPSHOT_LAUNCHER="$SNAPSHOT_SOURCE_ROOT/infra/azure/scripts/10_run_parser_v3_locked_eval.sh"
