#!/usr/bin/env bash
# Launch the Phase 1.0C Track B capability/headroom calibration generation run
...
RUN_ID="${JSPACE_HEADROOM_RUN_ID:-$(date -u +'%Y%m%dT%H%M%SZ')}"
...
if [[ ! "$IMAGE_DIGEST" =~ ^sha256:[0-9a-f]{64}$ ]]; then
    echo "[FAIL] No immutable calibration image is tagged $PROJECT_SHA"
    exit 1
fi
...
if [[ "${TAG_WRITE_ENABLED,,}" != "false" \
    ...
    echo "[FAIL] Calibration image is not locked; refusing to launch"
    exit 1
fi
...
RECORD_DIR="${HEADROOM_RUN_RECORD_DIR:-$PROJECT_ROOT/results/runs/phase1-headroom-${RUN_ID}}"
mkdir -p "$RECORD_DIR"
...
COMMAND="timeout --signal=TERM --kill-after=60s ${GENERATION_TIMEOUT_SECONDS}s python /workspace/scripts/run_phase1_headroom_calibration.py --mode generate --output-root /workspace/runtime/results --upload-blob"
...
environment = [
    ...
    {"name": "JSPACE_HEADROOM_RUN_ID", "value": "$RUN_ID"},
...
body = {
    ...
    "tags": {
        ...
        "phase": "1.0C",
        "track": "B",
        "stage": "capability-headroom-calibration",
        "run-id": "$RUN_ID",
